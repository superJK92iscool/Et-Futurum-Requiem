...
/**
 * Modified to replace nylium and blackstone
 */
public class MapGenCavesHellModified extends MapGenBase {
	...
	protected void func_151543_a(long p_151543_1_, int p_151543_3_, int p_151543_4_, Block[] p_151543_5_, double p_151543_6_, double p_151543_8_, double p_151543_10_, float p_151543_12_, float p_151543_13_, float p_151543_14_, int p_151543_15_, int p_151543_16_, double p_151543_17_) {
		...
		for (boolean flag = random.nextInt(6) == 0; p_151543_15_ < p_151543_16_; ++p_151543_15_) {
			double d6 = 1.5 + (double) (MathHelper.sin((float) p_151543_15_ * 3.1415927F / (float) p_151543_16_) * p_151543_12_ * 1.0F);
			...
			if (!flag1 && p_151543_15_ == k1 && p_151543_12_ > 1.0F) {
				this.func_151543_a(random.nextLong(), p_151543_3_, p_151543_4_, p_151543_5_, p_151543_6_, p_151543_8_, p_151543_10_, random.nextFloat() * 0.5F + 0.5F, p_151543_13_ - 1.5707964F, p_151543_14_ / 3.0F, p_151543_15_, p_151543_16_, 1.0);
				this.func_151543_a(random.nextLong(), p_151543_3_, p_151543_4_, p_151543_5_, p_151543_6_, p_151543_8_, p_151543_10_, random.nextFloat() * 0.5F + 0.5F, p_151543_13_ + 1.5707964F, p_151543_14_ / 3.0F, p_151543_15_, p_151543_16_, 1.0);
				return;
			}
			...
			if (flag1 || random.nextInt(4) != 0) {
				...
				if (p_151543_6_ >= d4 - 16.0 - d6 * 2.0 && p_151543_10_ >= d5 - 16.0 - d6 * 2.0 && p_151543_6_ <= d4 + 16.0 + d6 * 2.0 && p_151543_10_ <= d5 + 16.0 + d6 * 2.0) {
					...
					if (!flag2) {
						for (k2 = i4; k2 < l1; ++k2) {
							double d13 = ((double) (k2 + p_151543_3_ * 16) + 0.5 - p_151543_6_) / d6;

							for (j3 = k4; j3 < j2; ++j3) {
								double d14 = ((double) (j3 + p_151543_4_ * 16) + 0.5 - p_151543_10_) / d6;
