		...
		if (stateName.equals("axis")) {
			switch (state) {
				case "y":
					return 0;
				case "x":
					return facing == ForgeDirection.NORTH || facing == ForgeDirection.SOUTH ? 4 : 8; //<= 4 is 2 and 3, north and sounth, other side would be 4 and 5, east and west.
				case "z":
					return facing == ForgeDirection.NORTH || facing == ForgeDirection.SOUTH ? 8 : 4;
			}
		}
		...
		if (stateName.equals("facing")) {
			switch (state) {
				...
				case "up":
					return 1;
				case "north":
					return ((facing.ordinal() - 2) % 4) + 2;
				case "south":
					return (((facing.ordinal() - 2) + 1) % 4) + 2;
				case "west":
					return (((facing.ordinal() - 2) + 2) % 4) + 2;
				case "east":
					return (((facing.ordinal() - 2) + 3) % 4) + 2;
			}
		}
		return 0;
	}
